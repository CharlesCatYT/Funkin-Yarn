...
class PlayState extends MusicBeatState
{
	...
	private function generateSong():Void
	{
		...
		#end
			var eventsData = Song.loadFromJson('events', songName).notes;
		...
		if(eventNotes.length > 1) { // no need to sort if theres a single one or none at all lol
	...
	function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			case 'Add Camera Zoom':
				if(PreferencesMenu.getPref('camera-zoom') && FlxG.camera.zoom < 1.35) {
